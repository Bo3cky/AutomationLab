...
createNodes() {
	...
	max=$(($idmax + $nb_machine))
        ...
        # Création de clés ssh et chargement de la clé privé dans l'agent-ssh
        ssh-keygen -f $HOME/.ssh/keys -q -N ""
	...
	for i in $(seq $min $max);do
		...
		docker exec -ti $USER-debian-$i /bin/sh -c "mkdir  ${HOME}/.ssh && chmod 700 ${HOME}/.ssh && chown $USER:$USER $HOME/.ssh"
                docker exec -ti $USER-debian-$i /bin/sh -c "touch  ${HOME}/.ssh/authorized_keys && chmod 700 ${HOME}/.ssh && chown $USER:$USER $HOME/.ssh/authorized_keys"
        docker cp $HOME/.ssh/keys.pub $USER-debian-$i:$HOME/.ssh/authorized_keys
...
dropNodes(){
	...
	docker rm -f $(docker ps -a | grep $USER-debian | awk '{print $1}')
        echo "Effacement de la clé ssh"
        sudo rm $HOME/.ssh/keys*
